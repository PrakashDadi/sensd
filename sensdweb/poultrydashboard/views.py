...
def _blu_creds(request) -> dict | None:
    credential = PoultryCredential.objects.filter(owner=request.user).first()
    if not credential:
        return None
    return {
        "uname": credential.username,
        "upass": credential.password,
        "demo": credential.is_demo,
    }
...
def _require_blu(view_func):
    def wrapper(request, *args, **kwargs):
        if not _blu_creds(request):
            return redirect(f"/poultry/connect/?next={request.path}")
...
@login_required
def home(request):
    profile = PoultryProfile.objects.filter(owner=_owner_key(request)).first()
    return render(request, "poultrydashboard/home.html", {"profile": profile})


@ensure_csrf_cookie
@login_required
def faq(request):
    return render(request, "poultrydashboard/faq.html")


@ensure_csrf_cookie
@login_required
def connect(request):
    return render(request, "poultrydashboard/login.html")
...
def sensor_feed(request):
    return render(request, "poultrydashboard/sensor_feed.html")
...
def visualizations(request):
    return render(request, "poultrydashboard/visualizations.html")
...
@_require_blu
def ai(request):
    return render(request, "poultrydashboard/ai.html")
...
@_require_blu
def ai_chat(request):
    return render(request, "poultrydashboard/ai_chat.html")


@ensure_csrf_cookie
@login_required
def contact(request):
    return render(request, "poultrydashboard/contact.html")
...
def api_blu_login(request):
    ...
    if (
        ...
        PoultryCredential.objects.update_or_create(
            owner=request.user,
            defaults={"username": uname, "password": upass, "is_demo": True},
        )
        return JsonResponse({"ok": True, "demo": True})
    try:
        bluconsole.blu_login(uname, upass)
    except Exception as exc:  # noqa: BLE001
        return JsonResponse({"error": "BluConsole authentication failed"}, status=401)
    PoultryCredential.objects.update_or_create(
        owner=request.user,
        defaults={"username": uname, "password": upass, "is_demo": False},
    )
...
def api_blu_logout(request):
    PoultryCredential.objects.filter(owner=request.user).delete()
    return JsonResponse({"ok": True})
...
def api_blu_devices(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Unable to retrieve BluConsole devices"}, status=502)
...
def api_blu_measurements(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Unable to retrieve BluConsole measurements"}, status=502)
...
def api_profile(request):
    ...
    if request.method == "GET":
        profile = PoultryProfile.objects.filter(owner=owner_key).first()
        if not profile:
    ...
    data = _json_body(request)
    profile, _ = PoultryProfile.objects.get_or_create(owner=owner_key)
...
def api_notes(request):
    ...
    if request.method == "GET":
        notes = (
            Note.objects.filter(owner=owner_key)
    ...
    note = Note.objects.create(owner=owner_key, title=title, body=body)
...
def api_note_detail(request, note_id: int):
    owner_key = _owner_key(request)
    note = Note.objects.filter(owner=owner_key, id=note_id).first()
    if not note:
...
def api_uploads(request):
    ...
    if request.method == "GET":
        uploads = (
            UploadDataset.objects.filter(owner=owner_key)
            .order_by("-created_at")
...
def api_uploads_clear(request):
    owner_key = _owner_key(request)
    UploadDataset.objects.filter(owner=owner_key).delete()
    return JsonResponse({"ok": True})
...
def api_upload_detail(request, upload_id: int):
    owner_key = _owner_key(request)
    upload = UploadDataset.objects.filter(owner=owner_key, id=upload_id).first()
    if not upload:
        return JsonResponse({"error": "Not found"}, status=404)
    if request.method == "DELETE":
        upload.delete()
...
def api_ai_chat_sessions(request):
    owner_key = _owner_key(request)
    sessions = (
        ChatSession.objects.filter(owner=owner_key)
        .order_by("-updated_at")
...
def api_ai_chat_session_detail(request, session_id: int):
    owner_key = _owner_key(request)
    session = ChatSession.objects.filter(owner=owner_key, id=session_id).first()
    if not session:
        return JsonResponse({"error": "Not found"}, status=404)
    messages = (
...
def api_ai_chat_sessions_clear(request):
    owner_key = _owner_key(request)
    ChatSession.objects.filter(owner=owner_key).delete()
    return JsonResponse({"ok": True})
...
def _get_or_create_chat_session(request, session_id: Any, prompt: str) -> ChatSession:
    ...
    return ChatSession.objects.create(owner=owner_key, title=title)
