(() => {
  ...
  if (upload) {
    upload.addEventListener("change", async () => {
      ...
      try {
        ...
        fd.append("file", file);
        const res = await window.BluDash.csrfFetch("/poultry/api/ai-chat/attachment/", {
          method: "POST",
          body: fd,
        });
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const res = await window.BluDash.csrfFetch("/poultry/api/ai-chat/", {
          method: "POST",
  ...
  const loadStatus = async () => {
    ...
    try {
      const res = await fetch("/poultry/api/ai-chat/status/");
      const data = await res.json();
  ...
  const loadSessions = async () => {
    ...
    try {
      const res = await fetch("/poultry/api/ai-chat/sessions/");
      const data = await res.json();
      ...
      sessions.forEach((s) => {
        ...
        btn.innerHTML = `
          <div class="text-sm font-medium text-slate-800">${escapeHtml(s.title)}</div>
          <div class="text-xs text-slate-500">${escapeHtml(formatDt(s.updated_at))}</div>
        `;
  ...
  const loadSessionMessages = async (sessionId) => {
    clearLog();
    try {
      const res = await fetch(`/poultry/api/ai-chat/sessions/${sessionId}/`);
      const data = await res.json();
  ...
  if (clearChatsBtn) {
    clearChatsBtn.addEventListener("click", async () => {
      ...
      try {
        await window.BluDash.csrfFetch("/poultry/api/ai-chat/sessions/clear/", { method: "DELETE" });
        currentSessionId = null;
        clearLog();
        await loadSessions();
