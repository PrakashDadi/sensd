(() => {
  const escapeHtml = window.BluDash.escapeHtml;
  ...
  const renderDevices = () => {
    ...
    devices.forEach((d) => {
      ...
      row.innerHTML = `
        <td class="px-3 py-2 border-b border-slate-200/70 text-slate-800">${escapeHtml(d.id ?? "-")}</td>
        <td class="px-3 py-2 border-b border-slate-200/70 text-slate-800 uppercase">${escapeHtml(d.type ?? "-")}</td>
        <td class="px-3 py-2 border-b border-slate-200/70 text-slate-800">${escapeHtml(d.label ?? "-")}</td>
        ...
        <td class="px-3 py-2 border-b border-slate-200/70">
          <a href="/poultry/visualizations/?id=${encodeURIComponent(String(d.id ?? ""))}&type=${d.type}" class="inline-flex items-center justify-center w-8 h-8 rounded-md border border-sky-500 text-sky-600 bg-white hover:bg-sky-50" title="View chart">
  ...
  const loadDevices = async () => {
    ...
    try {
      const res = await fetchJson("/poultry/api/blu/devices/");
    ...
    const worker = async () => {
      while (idx < devices.length) {
        ...
        try {
          ...
          const res = await fetchJson(`/poultry/api/blu/measurements/?${qs.toString()}`);
  ...
  const uploadFile = async (file) => {
    ...
    fd.append("file", file);
    const res = await window.BluDash.csrfFetch("/poultry/api/uploads/", {
      method: "POST",
      body: fd,
    });
    let data = null;
  ...
  const loadHistory = async () => {
    const res = await fetchJson("/poultry/api/uploads/");
    ...
    uploads.forEach((u) => {
      ...
      tr.innerHTML = `
        ...
        <td class="px-3 py-2 border-b border-slate-200/70 text-slate-800 whitespace-nowrap">
          ${escapeHtml(new Date(u.created_at).toLocaleString())}
        </td>
      `;
      tr.addEventListener("click", async () => {
        const detail = await fetchJson(`/poultry/api/uploads/${u.id}/`);
  ...
  if (clearHistory) {
    clearHistory.addEventListener("click", async () => {
      ...
      await window.BluDash.csrfFetch("/poultry/api/uploads/clear/", { method: "DELETE" });
      clearPreviewTable();
      await loadHistory();
    });
  }
