(() => {
      ...
      card.innerHTML = `
        <div class="flex items-center justify-between">
          <div class="text-sm text-slate-700">
            ...
            <span class="font-medium">${escapeHtml(a.id || "-")}</span>
            <span class="uppercase text-slate-500">(${escapeHtml(a.type)})</span>
          </div>
        ...
        <div class="mt-2 grid grid-cols-2 gap-2 text-sm text-slate-800">
          ...
          <div>
            ...
            <div><span class="text-slate-600">Location:</span> ${escapeHtml(a.label || a.org || "-")}</div>
            <div><span class="text-slate-600">Battery:</span> ${a.battery != null ? `${a.battery}%` : "-"}</div>
          </div>
        </div>
        <div class="mt-3">
          <a href="/poultry/sensor-feed/" class="text-auburn underline hover:no-underline text-sm">Open in Sensor Feed</a>
        </div>
      `;
  ...
  const renderStatus = (items) => {
    ...
    items.forEach((s) => {
      ...
      row.innerHTML = `
        ...
        <td class="px-3 py-2">${escapeHtml(s.lastUpdate || "-")}</td>
        <td class="px-3 py-2">
  ...
  const load = async () => {
    try {
      const status = await fetchJson("/poultry/api/blu/status/");
      if (!status.authenticated) {
        setHeader(false);
        return;
      }
      const profileData = await fetchJson("/poultry/api/profile/");
      ...
      const devicesRes = await fetchJson("/poultry/api/blu/devices/");
      ...
      const worker = async () => {
        while (index < devices.length) {
          ...
          try {
            ...
            const meas = await fetchJson(`/poultry/api/blu/measurements/?${q.toString()}`);
